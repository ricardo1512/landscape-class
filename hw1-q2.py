...
class LogisticRegression(nn.Module):
    ...
    def __init__(self, n_classes, n_features, **kwargs):
        ...
        self.weights = nn.Parameter(torch.randn(n_features, n_classes) * 0.01)  # Initialize weights
        self.bias = nn.Parameter(torch.zeros(n_classes))  # Initialize bias
...
class FeedforwardNetwork(nn.Module):
    def __init__(
        ...
        """
        n_classes (int), n_features (int), hidden_size (int)
        layers (int)
        activation_type (str)
        ...
        """
        super().__init__()
        # Select activation function based on the input type.
        activations = {
            'relu': nn.ReLU,
            'tanh': nn.Tanh,
            'sigmoid': nn.Sigmoid
...
def train_batch(X, y, model, optimizer, criterion, **kwargs):
    ...
    # Forward pass
    logits = model(X)  # Compute model's output
...
def plot(epochs, plottables, learning_rate, model, filename=None, ylim=None):
    ...
    plt.legend()
    plt.title(f'{model} with dropout of {learning_rate}')
    if ylim:
        plt.ylim(ylim)
    if filename:
...
def main():
    ...
    print("Final validation acc: {:.4f}".format(valid_accs[-1]))
    ...
    plot(epochs, losses, opt.dropout, opt.model, filename=f'images/2/{opt.model}-training-loss-{config}.png')
    accuracy = { "Valid Accuracy": valid_accs }
    plot(epochs, accuracy, opt.dropout, opt.model, filename=f'images/2/{opt.model}-validation-accuracy-{config}.png')
